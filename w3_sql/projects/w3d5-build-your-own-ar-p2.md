...
# override defaults
options = BelongsToOptions.new(:human, :class_name => "Human")
options.class_name # => "Human"
```

Use the inflector's `String#camelcase`, `String#singularize` to aid
you in your quest.
...
options of a `belongs_to` association.

Modify your `04_associatiable` file and implement the `::assoc_options`
class method. It should start out by initializing a class instance
variable with a blank hash. Modify your `belongs_to` method to save
the `BelongsToOptions` in the `assoc_options` hash, setting the options
as the value for the key `name`:
...
`cat.house` **after** the declaration of the `Human` and `House`
